 ...
 */
public class CreativeDeliveryCampaignHierarchy extends BaseClass {

    private static final String REPORT_HEADER = "Creative Delivery: Hierarchical Level Test <BR> Measurement Sum Comparison Between SQL NAN And Datorama <BR> Group By:  CAMPAIGN ID";
    ...
    public void creativeLevelCampaignTest() throws Exception {
        ...
        List<String> creativeLevelSQLList = executor.executeQuery(sqlFile, envt, "getCreativeMeasurementCampaign", "$ColumnName$", "campaign_id", "$START_DATE$", reportStartDate, "$END_DATE$", reportEndDate);
        ...
        String Resp = APIUtil.getResportAsString("/query/execBatchQuery",APIRequestBodyGenerator.getCreativeDeliveryLevelCampaign(reportStartDate, reportEndDate, brandId), token);
        ...
        Map<String, DestinationTable> mapper = etlUtil.getMapSet(System.getProperty("user.dir")+"/"+"Datorama_Creative_Hierarchcial.xlsx", "CreativeDelivery_CampaignID");
