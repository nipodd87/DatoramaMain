...
/**
 * Created by nitin.poddar on 3/23/2017.
 */
public class CreativeDeliveryHierarchyPlacementId extends BaseClass {

    private static final String REPORT_HEADER = "Creative Delivery: Hierarchical Level Test <BR> Measurement Sum Comparison Between SQL NAN And Datorama <BR> Group By:  ADSERVER PLACEMENT ID";
    private static final String REPORT_TITLE = "This test is to compare at different hierarchical levels for ADSERVER PLACEMENT ID to test the counts between SQL NAN and Datorama API Endpoints";
    ...
    @Test
    public void creativeLevelPlacementLevelTest() throws Exception {
        ...
        List<String> creativeLevelSQLList = executor.executeQuery(sqlFile, envt, "getCountCreativeDeliveryForPlacementId", "$ColumnName$", "Adserver_Placement_ID", "$START_DATE$", reportStartDate, "$END_DATE$", reportEndDate);
        ...
        String Resp = APIUtil.getResportAsString("/query/execBatchQuery",APIRequestBodyGenerator.getCreativeDeliveryLevelPublisher(reportStartDate, reportEndDate, brandId), token);
