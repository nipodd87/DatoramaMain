 ...
 */
public class TraitDeliveryCampaignHierarchy extends BaseClass {
    private static final String REPORT_HEADER = "Trait Delivery: Hierarchical Level Test <BR> Measurement Sum Comparison Between SQL NAN And Datorama <BR> Group By:  CAMPAIGN ID";
    private static final String REPORT_TITLE = "This test is to compare at different hierarchical levels for CAMPAIGN to test the counts between SQL NAN and Datorama API Endpoints";
    ...
    @Test
    public void traitLevelCampaignTest() throws Exception {
        ...
        List<String> traitLevelSQLList = executor.executeQuery(sqlFile, envt, "getCountTraitDeliveryLevelByCampaign", "$ColumnName$", "campaign_id", "$START_DATE$", reportStartDate, "$END_DATE$", reportEndDate);
        ...
        String Resp = APIUtil.getResportAsString("/query/execBatchQuery",APIRequestBodyGenerator.getTraitDeliveryLevelCampaign(reportStartDate, reportEndDate, brandId), token);
        ...
        Map<String, DestinationTable> mapper = etlUtil.getMapSet(System.getProperty("user.dir")+"/"+"Datorama_Trait_Hierarchcial.xlsx", "TraitDelivery_CampaignID");
